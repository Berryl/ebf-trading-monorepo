...
class TestLoadConfig:
    ...
    def test_calls_svc_and_returns_cfg_only_when_return_sources_is_false(self, monkeypatch) -> None:
        ...
        assert_load_called_once_with_args(
            m, "app", project_search_path="config", filename=None, user_filename="user.yml",
    ...
    def test_calls_svc_and_returns_sources_also_when_return_sources_is_true(self, monkeypatch) -> None:
        ...
        result = sut.load_config(app_name="app", project_search_path="config", filename="project.yml",
                                 user_filename="user.yml", return_sources=True, file_util=None, )

        assert result == (cfg, sources)
        assert_load_called_once_with_args(
            m, "app", project_search_path="config", filename="project.yml", user_filename="user.yml",
            return_sources=True, file_util=None, )

    def test_exceptions_propagate(self, monkeypatch) -> None:
        m = mocked_cfg_service(monkeypatch, sut, side_effect=RuntimeError("blah"))

        with pytest.raises(RuntimeError, match="blah"):
            sut.load_config(app_name="app", filename="project.yml")

        # optional: ensure it actually tried the call
        assert_load_called_once_with_args(
            m, "app", project_search_path="config", filename="project.yml",
            user_filename=None, return_sources=False, file_util=None,)
