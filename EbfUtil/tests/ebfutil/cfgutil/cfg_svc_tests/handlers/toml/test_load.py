...
class TomlConfigServiceFixture(ConfigServiceFixture):
    ...
    @pytest.fixture(scope="class")
    def toml_cfg_file(self, make_filename) -> str:
        return make_filename()

    @pytest.fixture
    def project_file(self, toml_cfg_file: str, project_root: Path, project_search_path: str, data: dict) -> Path:
        tgt = project_root / project_search_path / toml_cfg_file
        tgt.parent.mkdir(parents=True, exist_ok=True)
        tgt.write_text(json.dumps(data), encoding="utf-8")
        return tgt

    @staticmethod
    def test_fixture_overrides(toml_cfg_file):
